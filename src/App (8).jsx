...
function Addresses({ products, addresses, setAddresses, notify, openScanner, sb, loadAll }) {
  ...
  return (
    <div className="anim">
      ...
      {/* Recherche adresse + scan */}
      <div style={{display:"flex",gap:8,marginBottom:10}}>
        <div style={{position:"relative",flex:1}}>
          <div style={{position:"absolute",left:14,top:"50%",transform:"translateY(-50%)",color:C.greyL}}><Ic d={D.srch} size={18} sw={2}/></div>
          <input className="field" style={{paddingLeft:44}} placeholder="Rechercher une adresse…" value={searchAddr} onChange={e=>setSearchAddr(e.target.value)}/>
        </div>
        <button className="btn-scan" onClick={()=>openScanner(scanned=>{const clean=scanned.replace(/[()\[\]]/g,'').trim();setSearchAddr(clean);notify(`Adresse : ${clean}`)})}>
          <Ic d={D.camera} size={16} sw={2}/>Scan
        </button>
