...
const CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent}
  ...
  .btn-export{background:#e8f5e9;border:1.5px solid #4caf50;color:#2e7d32;border-radius:10px;padding:10px 16px;font-weight:800;font-size:13px;cursor:pointer;display:flex;align-items:center;gap:6px;white-space:nowrap}
  ...
  .qty-btn-minus{background:#eef0f9;border:none;border-radius:8px;width:44px;height:44px;cursor:pointer;font-weight:900;font-size:26px;color:#1a2b6b;display:flex;align-items:center;justify-content:center;flex-shrink:0;line-height:1}
  .qty-btn-plus{background:#1a2b6b;border:none;border-radius:8px;width:44px;height:44px;cursor:pointer;font-weight:900;font-size:26px;color:white;display:flex;align-items:center;justify-content:center;flex-shrink:0;line-height:1}
  .qty-input{width:76px;background:#f7f8fd;border:2px solid #dde0ee;border-radius:8px;padding:8px 6px;font-size:22px;font-weight:900;color:#1a2b6b;outline:none;text-align:center;font-family:'Nunito',sans-serif}
  ...
  .confirm-box{background:white;border-radius:20px;padding:24px;width:100%;max-width:380px;box-shadow:0 20px 60px rgba(10,15,40,.3)}
...
function CameraScanner({ onScan, onClose }) {
  ...
  useEffect(() => {
    let active = true;
    const hints = new Map();
    const formats = [0,1,2,3,4,5,6,7,8,11,14,15];
    hints.set(2, formats);
    ...
    let t1 = setTimeout(()=>{ if(active) setHint("Rapprochez ou éloignez le téléphone"); }, 3000);
    let t2 = setTimeout(()=>{ if(active) setHint("Essayez d'améliorer l'éclairage"); }, 8000);
    ...
    return () => { active=false; clearTimeout(t1); clearTimeout(t2); try{reader.reset();}catch{} };
  }, [onScan]);
  ...
  return (
    <div className="scanner-modal">
      <div style={{textAlign:"center",padding:"0 20px 24px"}}>
        ...
        <div style={{color:"rgba(255,255,255,.75)",fontSize:14,fontWeight:700}}>{hint}</div>
      </div>
      {error ? (
...
export default function StockApp() {
  ...
  const handleImportProducts = async (data, mode) => {
    ...
    try {
      ...
      const BATCH=200; const total=deduped.length; let imported=0;
      for(let i=0;i<deduped.length;i+=BATCH){
        ...
        if(imported%2000===0||imported===total) notify(`⏳ ${imported}/${total} produits importés…`);
...
function Products({ products, productCount, addresses, notify, openScanner, sb, loadAll, searchProducts }) {
  ...
  const list = q.length>=2 ? (searchResults||[]) : [];
    ...
    <div className="anim">
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        <div>
          ...
          <p style={{color:C.grey,fontSize:13,fontWeight:600,marginTop:2}}>{productCount.toLocaleString()} références</p>
        </div>
...
function Addresses({ products, addresses, setAddresses, notify, openScanner, sb, loadAll }) {
  ...
  const exportExcel = async () => {
    ...
    try {
      const allEans = [...new Set(Object.values(addresses).flatMap(a=>a.products.map(p=>p.ean)))];
      const prodMap = {};
      const BATCH = 200;
      for(let i=0; i<allEans.length; i+=BATCH){
        const batch = allEans.slice(i, i+BATCH);
        const{data} = await sb.from("products").select("ean,name,unit").in("ean", batch);
        (data||[]).forEach(p=>{ prodMap[p.ean]=p; });
      }

      const dataRows = [];
      Object.keys(addresses).sort().forEach(addr=>{
        const prods = addresses[addr]?.products||[];
        prods.forEach(({ean:e,qty:q})=>{
          const prod = prodMap[e];
          // Police EAN13 : les 13 chiffres bruts suffisent
          dataRows.push([addr, String(e), String(e), prod?.name||"Inconnu", Number(q), prod?.unit||"pcs"]);
        });
      });
      ...
      const wsData = [["Adresse","EAN","Code Barre (police EAN13)","Nom produit","Quantité","Unité"], ...dataRows];
      const ws = XLSX2.utils.aoa_to_sheet(wsData);

      // Forcer EAN et Code Barre en texte (évite notation scientifique)
      for(let i=1; i<=dataRows.length; i++){
        ["B","C"].forEach(col=>{
          const ref = col+String(i+1);
          if(ws[ref]){ ws[ref].t="s"; ws[ref].z="@"; }
        });
      }

      // Largeurs colonnes : Code Barre plus large pour que le barcode soit lisible
      ws["!cols"]=[{wch:10},{wch:18},{wch:32},{wch:42},{wch:10},{wch:8}];

      // Hauteur des lignes : plus haute pour que le code-barres soit visible
      ws["!rows"] = [{hpt:20}]; // header
      for(let i=1; i<=dataRows.length; i++){
        ws["!rows"].push({hpt:36}); // lignes données — hauteur 36pt pour EAN13
      }

      XLSX2.utils.book_append_sheet(wb, ws, "Stock Adresses");
      XLSX2.writeFile(wb, `stock_adresses_${new Date().toISOString().slice(0,10)}.xlsx`);
      notify(`✅ Export téléchargé — ${dataRows.length} ligne(s)`);
    } catch(err){ notify("Erreur export : "+err.message,"error"); }
  }
  ...
  // ── Gestion quantités : +/- sans confirmation immédiate ────────────────────
  const requestQtyChange = (addr, eanVal, currentDisplayQty, originalQty) => {
    // currentDisplayQty = valeur actuellement affichée (déjà modifiée)
    if(currentDisplayQty < 0) return;
    setPendingQty(prev => ({
      ...prev,
      [eanVal]: {
        qtyOriginal: prev[eanVal]?.qtyOriginal ?? originalQty,
        qtyDisplay: currentDisplayQty,
        addr
      }
    }));
    setAddresses(prev=>({...prev,[addr]:{...prev[addr],products:prev[addr].products.map(p=>p.ean===eanVal?{...p,qty:currentDisplayQty}:p)}}));
  };

  const doConfirmQty = async () => {
    setConfirmModal(false);
    for(const [eanVal, {qtyOriginal, qtyDisplay, addr}] of Object.entries(pendingQty)){
      let prodName = products.find(p=>p.ean===eanVal)?.name || eanVal;
      if(prodName===eanVal){
        const{data}=await sb.from("products").select("name").eq("ean",eanVal).limit(1);
        if(data&&data.length>0) prodName=data[0].name;
      }
      if(Number(qtyDisplay)===0){
        await sb.from("addresses").delete().eq("address",addr).eq("ean",eanVal);
      } else {
        await sb.from("addresses").update({qty:Number(qtyDisplay)}).eq("address",addr).eq("ean",eanVal);
      }
      await addHistory("modification",eanVal,prodName,addr,qtyOriginal,Number(qtyDisplay));
    }
    setPendingQty({});
  ...
  const handleQtyInput=(eanVal,val,addr,originalQty)=>{
    setPendingQty(prev=>({
      ...prev,
      [eanVal]:{ qtyOriginal: prev[eanVal]?.qtyOriginal ?? originalQty, qtyDisplay: val, addr }
    }));
    setAddresses(prev=>({...prev,[addr]:{...prev[addr],products:prev[addr].products.map(p=>p.ean===eanVal?{...p,qty:val}:p)}}));
  };

  return (
    <div className="anim">

      {/* Modale confirmation groupée */}
      {confirmModal&&(
          ...
          <div className="confirm-box anim">
            ...
            <div style={{maxHeight:"40vh",overflowY:"auto",marginBottom:16}}>
              {Object.entries(pendingQty).map(([eanVal,{qtyOriginal,qtyDisplay}])=>{
                const diff=Number(qtyDisplay)-Number(qtyOriginal);
                return(
                  <div key={eanVal} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 12px",marginBottom:6,background:C.light,borderRadius:10}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
                      <span style={{fontSize:14,color:C.grey}}>{qtyOriginal}</span>
                      <span style={{fontSize:14,color:C.grey}}>→</span>
                      <span style={{fontSize:18,fontWeight:900,color:diff>=0?C.green:C.red}}>{qtyDisplay}</span>
                      <span style={{fontSize:13,fontWeight:800,color:diff>=0?C.green:C.red,minWidth:32}}>{diff>0?"+":""}{diff}</span>
                    </div>
                  </div>
                );
              })}
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
        ...
        <button className="btn-export" onClick={exportExcel}>
          <Ic d={D.download} size={15} sw={2}/>Export Excel
        </button>
      </div>
          ...
          <div className="detail-content">
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
              <div>
                ...
                <div style={{color:C.grey,fontSize:12,fontWeight:700,marginTop:6}}>
                  {selData.products.length} produit{selData.products.length!==1?"s":""} · {selData.products.reduce((s,p)=>s+Number(p.qty),0)} unités
                </div>
              </div>
            ...
            <div style={{marginBottom:14}}>
              {selData.products.length===0?(
                ...
                const displayQty=isPending?pendingQty[e].qtyDisplay:q;
                return (
                  <div key={e} style={{padding:"12px",marginBottom:8,background:C.light,borderRadius:12,border:`1.5px solid ${isPending?C.orange:C.border}`}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:10}}>
                      <button className="qty-btn-minus" onClick={()=>requestQtyChange(sel,e,Number(displayQty)-1,q)}>−</button>
                      <input
                      ...
                      />
                      <button className="qty-btn-plus" onClick={()=>requestQtyChange(sel,e,Number(displayQty)+1,q)}>+</button>
                      <span style={{fontSize:13,color:C.grey,fontWeight:700}}>{prod?.unit||"pcs"}</span>
                    </div>
                  </div>
                );
              })}
