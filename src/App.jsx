    ...
    <div className="scanner-modal">
      ...
      {error ? (
        <div style={{background:"white",borderRadius:16,padding:24,margin:20,textAlign:"center",width:"calc(100% - 40px)"}}>
          <div style={{color:C.red,fontWeight:700,marginBottom:16,fontSize:15}}>{error}</div>
...
function normalizeRow(row) { const n={}; Object.keys(row).forEach(k=>{n[k.toLowerCase().replace(/[^a-z0-9]/g,"")]=row[k];}); return n; }
  ...
  return (
    <div className="modal-bg" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal">
        ...
        {loading&&<div style={{textAlign:"center",padding:"20px 0",color:C.grey,fontWeight:700}}>⏳ Lecture…</div>}
        ...
        {preview&&!loading&&(
          <div className="anim">
            <div style={{background:"#e6f9ee",borderRadius:10,padding:"10px 14px",marginTop:12,border:"1.5px solid #b8edcc",marginBottom:14}}>
              <span style={{fontWeight:800,fontSize:13,color:"#1a7a3e"}}>✅ {count} {isProducts?"produit":"emplacement"}{count>1?"s":""} prêts</span>
            </div>
...
export default function StockApp() {
  ...
  const handleImportProducts=(data,mode)=>{ if(mode==="replace"){setProducts(data);notify(`${data.length} produits importés`);}else{const merged=[...products];let added=0,updated=0;data.forEach(p=>{const idx=merged.findIndex(x=>x.ean===p.ean);if(idx>=0){merged[idx]=p;updated++;}else{merged.push(p);added++;}});setProducts(merged);notify(`${added} ajouté${added>1?"s":""}, ${updated} mis à jour`);} setImportModal(null); };
  const handleImportAddresses=(addrMap,mode)=>{ setAddresses(prev=>{const next=mode==="replace"?(()=>{const a={};for(let i=1;i<=99;i++)a[`100-${String(i).padStart(2,"0")}`]={products:[]};return a;})():{...prev};Object.entries(addrMap).forEach(([addr,prods])=>{if(!next[addr])next[addr]={products:[]};if(mode==="replace")next[addr].products=prods;else prods.forEach(p=>{const ex=next[addr].products.findIndex(x=>x.ean===p.ean);if(ex>=0)next[addr].products[ex].qty=p.qty;else next[addr].products.push(p);});});return next;}); notify(`${Object.keys(addrMap).length} emplacements importés`); setImportModal(null); };
...
function Products({ products, setProducts, addresses, notify, onImport, openScanner }) {
  ...
  const add=()=>{ if(!form.ean||!form.name)return notify("EAN et nom requis","error"); if(products.find(p=>p.ean===form.ean))return notify("Cet EAN existe déjà","error"); setProducts([...products,{...form,stock:Number(form.stock)}]); setForm({ean:"",name:"",category:"",stock:0,unit:"pcs"}); setShowForm(false); notify(`"${form.name}" ajouté`); };
...
function Addresses({ products, addresses, setAddresses, notify, onImport, openScanner }) {
  ...
  const addProd=()=>{ if(!sel||!ean.trim())return; const prod=products.find(p=>p.ean===ean.trim()); if(!prod)return notify(`EAN "${ean}" introuvable`,"error"); if(addresses[sel].products.find(p=>p.ean===ean.trim()))return notify("Produit déjà présent","error"); setAddresses(prev=>({...prev,[sel]:{...prev[sel],products:[...prev[sel].products,{ean:ean.trim(),qty:Number(qty)||1}]}})); setEan("");setQty(1); notify(`"${prod.name}" ajouté à ${sel}`); };
  const rmProd=(addr,e)=>{ setAddresses(prev=>({...prev,[addr]:{...prev[addr],products:prev[addr].products.filter(x=>x.ean!==e)}})); notify("Produit retiré"); };
  const updateQty=(addr,eanVal,val)=>{ const q=Math.max(0,Number(val)||0); if(q===0)return rmProd(addr,eanVal); setAddresses(prev=>({...prev,[addr]:{...prev[addr],products:prev[addr].products.map(p=>p.ean===eanVal?{...p,qty:q}:p)}})); };
    ...
    <div className="anim">
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
        <div>
          ...
          <p style={{color:C.grey,fontSize:13,fontWeight:600,marginTop:2}}>{used}/{keys.length} occupés</p>
        </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
        ...
        <div className="card" style={{padding:"12px 16px",textAlign:"center"}}>
          <div style={{fontSize:28,fontWeight:900,color:C.greyL}}>{keys.length-used}</div>
