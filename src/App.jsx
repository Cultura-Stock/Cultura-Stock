...
export default function StockApp() {
  ...
  const loadAll = async () => {
    ...
    try {
      // Charger produits
      const { data: prods, error: e1 } = await sb.from("products").select("*").order("name");
      if (e1) throw e1;
      setProducts(prods || []);

      // Charger adresses
      const { data: addrs, error: e2 } = await sb.from("addresses").select("*");
      if (e2) throw e2;

      // Reconstruire la map d'adresses
      const addrMap = {};
      (addrs || []).forEach(({ address, ean, qty }) => {
  ...
  const handleImportAddresses = async (addrMap, mode) => {
    ...
    try {
      ...
      Object.entries(addrMap).forEach(([address, prods]) => {
        prods.forEach(({ ean, qty }) => rows.push({ address, ean, qty }));
      });
      const BATCH = 500;
