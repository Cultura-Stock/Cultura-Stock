...
function Addresses({ products, addresses, setAddresses, notify, openScanner, sb, loadAll }) {
  ...
  const requestQtyChange = (addr, eanVal, newQty, currentQty) => {
    ...
    if(newQty<0) return;
    setConfirmModal({addr,eanVal,qtyBefore:currentQty,qtyAfter:newQty,productName:prod?.name||eanVal});
  };

  const doConfirmQty = async () => {
    if(!confirmModal) return;
    const{addr,eanVal,qtyBefore,qtyAfter,productName}=confirmModal;
    setConfirmModal(null);
    if(qtyAfter===0) return rmProd(addr,eanVal);
    await sb.from("addresses").update({qty:qtyAfter}).eq("address",addr).eq("ean",eanVal);
    await addHistory("modification",eanVal,productName,addr,qtyBefore,qtyAfter);
    setAddresses(prev=>({...prev,[addr]:{...prev[addr],products:prev[addr].products.map(p=>p.ean===eanVal?{...p,qty:qtyAfter}:p)}}));
    notify("Quantité mise à jour ✅");
  };

  // Saisie manuelle : stocke localement
  const handleQtyInput=(eanVal,val)=>{ setPendingQty(prev=>({...prev,[eanVal]:val})); };
  const handleQtyBlur=(addr,eanVal,currentQty)=>{
    const raw=pendingQty[eanVal];
    if(raw===undefined) return;
    const newQty=Math.max(0,Number(raw)||0);
    setPendingQty(prev=>{const n={...prev};delete n[eanVal];return n;});
  ...
  return (
    <div className="anim">
      {confirmModal&&<ConfirmQtyModal
        product={confirmModal.productName} address={confirmModal.addr}
        qtyBefore={confirmModal.qtyBefore} qtyAfter={confirmModal.qtyAfter}
        onConfirm={doConfirmQty} onCancel={()=>setConfirmModal(null)}
      />}
                  ...
                  <div key={e} style={{padding:"12px",marginBottom:8,background:C.light,borderRadius:12,border:`1.5px solid ${isPending?C.orange:C.border}`}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                      ...
                      <input
                        ...
                        onChange={ev=>handleQtyInput(e,ev.target.value)}
                      ...
                      {isPending&&(
                        <button className="btn-confirm" onClick={()=>handleQtyBlur(sel,e,q)}>
                          <Ic d={D.check} size={13} sw={2.5}/>Valider
                        </button>
                      )}
                    </div>
                  </div>
