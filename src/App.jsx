            ...
            <div key={p.ean} className="card" style={{padding:"14px 16px"}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:4}}>
                ...
                <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                  {addrs.length===0?<span style={{color:C.greyL,fontSize:12,fontWeight:600}}>Non adressé</span>:addrs.map(({addr,qty})=><span key={addr} className="bdg b-navy">{addr} <span style={{opacity:.7}}>×{qty}</span></span>)}
                </div>
              </div>
            </div>
          );
...
function Addresses({ products, addresses, setAddresses, notify, onImport, openScanner, sb, loadAll }) {
  ...
  const [pendingQty,setPendingQty]=useState({}); // {ean: valeur en attente}
  ...
  const updateQtyLocal = (eanVal, val) => {
    setPendingQty(prev=>({...prev,[eanVal]:val}));
  };

  const confirmQty = async (addr,eanVal) => {
                  ...
                  <div key={e} style={{padding:"12px",marginBottom:8,background:C.light,borderRadius:12,border:`1.5px solid ${C.border}`}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
                      ...
                      <input className="qty-input" 
                        value={pendingQty[e]!==undefined ? pendingQty[e] : q} 
                        type="number" min="1" 
                        onChange={ev=>updateQtyLocal(e,ev.target.value)}
                        onBlur={()=>confirmQty(sel,e)}
                        style={{borderColor:pendingQty[e]!==undefined?"#e74c3c":"#dde0ee"}}
                      />
                      ...
                      {pendingQty[e]!==undefined && (
                        <button onClick={()=>confirmQty(sel,e)} style={{background:"#1a2b6b",border:"none",borderRadius:8,padding:"6px 12px",color:"white",fontFamily:"inherit",fontWeight:700,fontSize:12,cursor:"pointer"}}>
                          ✓ Valider
                        </button>
                      )}
                    </div>
