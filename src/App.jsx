...
export default function StockApp() {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      while(true){ const{data,error}=await sb.from("products").select("*").order("name").range(from,from+PAGE-1); if(error)throw error; allProds=[...allProds,...(data||[])]; if(!data||data.length<PAGE)break; from+=PAGE; }
      setProducts(allProds);

      let allAddrs=[]; from=0;
      while(true){ const{data,error}=await sb.from("addresses").select("*").range(from,from+PAGE-1); if(error)throw error; allAddrs=[...allAddrs,...(data||[])]; if(!data||data.length<PAGE)break; from+=PAGE; }
      const addrMap={};
  ...
  const handleImportProducts = async (data, mode) => {
    ...
    try {
      if(mode==="replace") await sb.from("products").delete().neq("id",0);
      ...
      for(let i=0;i<deduped.length;i+=BATCH){
        ...
        const{error}=await sb.from("products").upsert(batch,{onConflict:"ean"}); if(error)throw error;
      }
      await loadAll(); notify(`✅ ${deduped.length} produits importés !`);
    ...
    <div style={{minHeight:"100vh",background:C.offwhite}}>
      ...
      <header className="header">
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <div style={{background:"rgba(255,255,255,.12)",borderRadius:20,padding:"5px 12px",fontSize:11,color:"rgba(255,255,255,.7)",fontWeight:700}}>{products.length} réf.</div>
      ...
      <div className="main-wrap">
        {loading ? (
          ...
          ? <Products products={products} addresses={addresses} notify={notify} openScanner={openScanner} sb={sb} loadAll={loadAll}/>
          : tab==="addresses"
...
// ── VUE PRODUITS ───────────────────────────────────────────────────────────────
function Products({ products, addresses, notify, openScanner, sb, loadAll }) {
