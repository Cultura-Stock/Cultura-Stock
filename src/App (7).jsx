...
function Addresses({ products, addresses, setAddresses, notify, openScanner, sb, loadAll }) {
  ...
  // ── Export Excel ────────────────────────────────────────────────────────────
  cons  exportExcel = async () => {
    notify("⏳ Préparation de l'export…");
    try {
      // Récupérer tous les EAN présents dans les adresses
      const allEans = [...new Set(Object.values(addresses).flatMap(a=>a.products.map(p=>p.ean)))];
      // Charger les infos produits depuis Supabase par lots
      const prodMap = {};
      const BATCH = 200;
...
");
      ...
      a.href=url; a.download=`stock_adresses_${new Date().toISOString().slice(0,10)}.csv`;
      a.click(); URL.revokeObjectURL(url);
      notify(`✅ Export téléchargé — ${rows.length-1} ligne(s)`);
