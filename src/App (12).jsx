...
export default function StockApp() {
  ...
  const loadAll = async () => {
    setLoading(true);
    try {
      // Charger seulement le nombre total + les adresses au démarrage
      // Les produits sont chargés à la demande via la recherche
      const{count}=await sb.from("products").select("*",{count:"exact",head:true});
      setProductCount(count||0);

      // Charger les 1000 premiers produits pour l'affichage initial
      let allProds=[]; let from=0; const PAGE=1000;
      while(true){
      ...
      // Charger toutes les adresses (beaucoup moins nombreuses)
      let allAddrs=[]; from=0;
      while(true){
    ...
    } catch(err){ notify("Erreur de connexion","error"); }
    setLoading(false);
  };

  // Recherche produits dans Supabase (pour gros volumes)
...
function Products({ products, productCount, addresses, notify, openScanner, sb, loadAll, searchProducts }) {
  ...
  // Recherche Supabase pour gros volumes (>5000 produits)
  useEffect(()=>{
    if(!q||q.length<2){ setSearchResults(null); return; }
    const isLarge=(productCount||products.length)>5000;
    if(!isLarge){ setSearchResults(null); return; }
    setSearching(true);
  ...
  const list=searchResults!==null
    ?searchResults
    :products.filter(p=>p.ean.includes(q)||p.name.toLowerCase().includes(q.toLowerCase())||p.category.toLowerCase().includes(q.toLowerCase()));
  ...
  return (
    <div className="anim">
      ...
      <div style={{display:"flex",gap:8,marginBottom:14}}>
        ...
        <button className="btn-scan" onClick={()=>openScanner(scanned=>{const clean=scanned.replace(/[()\[\]]/g,'').trim();setQ(clean);notify(`Scanné : ${clean}`)})}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        {searching&&(
          <div className="card" style={{padding:"20px",textAlign:"center",color:C.grey}}>
            <div className="spinner" style={{marginBottom:10}}/><div style={{fontWeight:700,fontSize:13}}>Recherche dans {(productCount||products.length).toLocaleString()} produits…</div>
          </div>
        )}
        {!searching&&list.length===0&&(
          <div className="card" style={{padding:"40px 20px",textAlign:"center",color:C.greyL}}>
            <Ic d={D.srch} size={36} sw={1.5}/><br/><br/>
            <span style={{fontWeight:700,fontSize:14}}>{q?"Aucun produit trouvé":"Aucun produit — importez votre catalogue"}</span>
          </div>
        )}
...
function Addresses({ products, addresses, setAddresses, notify, openScanner, sb, loadAll }) {
  ...
  const rmProd = async (addr,e) => {
    ...
    await addHistory("suppression",e,prod?.name||e,addr,qtyBefore,0);
